...
/// Generate a single PDF containing several OMR answer sheets (gabaritos) from
/// a JSON-encoded array of `AnswerSheetSpec` objects.
///
/// Same calling convention, staging buffer, and error reporting as
/// [`prova_pdf_generate_answer_sheet`]; the input JSON must be an array.
#[cfg(feature = "answer-sheet")]
#[unsafe(no_mangle)]
pub extern "C" fn prova_pdf_generate_answer_sheets(
    json_ptr: *const u8,
    json_len: usize,
    out_buf:  *mut u8,
    out_cap:  usize,
    ...
    let specs: Vec<crate::spec::AnswerSheetSpec> = match serde_json::from_str(json_str) {
        Ok(s)  => s,
        Err(e) => { set_last_error(format!("JSON parse error: {e}")); return -1; }
    };

    let pdf_bytes = match generate_answer_sheets_from_specs(specs) {
        Ok(b)  => b,
        Err(e) => { set_last_error(e); return -1; }
    };
...
/// Run the batch answer-sheet pipeline against the thread-local registries.
#[cfg(feature = "answer-sheet")]
